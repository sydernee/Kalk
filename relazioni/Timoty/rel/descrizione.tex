...
Infine la classe offre una serie di operazioni insiemistiche tra due \texttt{Network}: 
\begin{itemize}[noitemsep]
...
input non validi.
% È presente nella classe un campo \texttt{stackedWidget} di tipo \texttt{QStackedWidget*},
% che si occuperà di tenere in memoria più widget contemporaneamente ma di tenerne attivo solamente uno.
% Il primo (\texttt{pages[0]}) è per la selezione delle matrici, mentre il secondo (\texttt{pages[1]}) è quello in cui c'è il calcolo effettivo delle matrici.
